...
describe('ImmutableDate', () => {
  test('create and is immutable', () => {
    ...
    expect(result.v.getTime()).toBe(nowTimestamp);
  });
  test('fromTimestamp', () => {
  ...
  test('fromTimestamp when invalid raw value', () => {
    ...
    expect(result.getErrorType()).toEqual('core.domain.value_object.immutable_date.invalid_raw_value');
    expect((result.value as AppError).data).toEqual({ msg: 'invalid timestamp', raw: -10 });
  });
});
